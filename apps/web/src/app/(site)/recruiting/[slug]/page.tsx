...
import { getPublicBoardThread } from '../../../../lib/api';
 ...
 *
 * These pages first declared `export const revalidate = 300`, reasoning that posts change
 ...
 * one indexed read, and it means a post edit is live immediately rather than up to five
...
export async function generateMetadata({
  ...
  const data = await getPublicBoardThread('recruiting', slug);
  if (data === null) return { title: "Post — Grim's Squad" };
  return {
    title: `${data.thread.title} — Grim's Squad`,
    description: `A step-by-step post from Grim's Squad: ${data.thread.title}.`,
  };
}
...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) {
   ...
   * allowed" page here would undo that by confirming the post exists.
   */
      ...
      */}
      ...
      <div className="space-y-10">
        {data.posts.map((post, i) => (
          ...
          >
            <div
              /*
               * `post-prose` carries the typography for server-rendered HTML we did not
      ...
      <footer className="mt-16 border-t border-[var(--color-border-hairline)] pt-6">
        <p className="text-sm leading-relaxed text-[var(--color-text-secondary)]">
          Something in this post wrong or out of date? Tell us in Discord — we would
