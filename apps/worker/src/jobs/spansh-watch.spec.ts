...
function harness(over: Partial<WatchDeps> & { projects?: WatchedProject[] } = {}) {
  ...
  const deps: WatchDeps = {
    ...
    /*
     * ★ THE OVERRIDE IS HONOURED, AND IT WAS NOT ★
     *
     * This read `close: async (p, since) => { closed.push(...) }` and never consulted `over.close`.
     * So "a close that throws does not take the sweep down with it" passed a throwing close, got
     * the recording one, and asserted the sweep survived something that never happened.
     *
     * The harness has to be as honest as the code it exercises: a test that cannot fail is worse
