...
export async function runWatchPass(
  ...
  for (const name of files) {
    ...
    const upload = await uploader.send(result.events);
    if (upload.unauthorised) {
      // Stop immediately, and do NOT advance. When the member re-pairs, this
      // picks up exactly where it left off.
      return {
        outcome: { ...empty(), filesRead, unauthorised: true, error: upload.error },
        config: next,
      };
    }
    ...
    if (!upload.ok) {
       ...
       */
      return { outcome: { ...empty(), filesRead, sent, duplicates, refused, error: upload.error }, config: next };
    }
  ...
  return { outcome: { filesRead, sent, duplicates, refused, unauthorised: false, error: null }, config: next };
}
...
function empty(): WatchOutcome {
  return { filesRead: 0, sent: 0, duplicates: 0, refused: {}, unauthorised: false, error: null };
}
