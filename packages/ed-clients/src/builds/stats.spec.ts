...
const SIDEWINDER: RawShipItem = {
  ...
  name: 'Sidewinder',
  data: {
    // Two small weapon mounts and four utility mounts, like the real hull — the shield and damage
    // tests need somewhere to fit a booster and a gun.
    slots: { standard: [2, 2, 2, 1, 1, 1, 2], hardpoints: [1, 1, 0, 0, 0, 0], internal: [2, 2, 1] },
    defaults: {
      standard: ['2E', '2E', 'O3', '1E', '1E', '1E', '2C'],
      hardpoints: [0, 0, 0, 0, 0, 0],
      internal: ['02', null, null],
    },
    bulkheads: [{ id: 'Bs', grp: 'bh', name: 'Lightweight Alloy', mass: 0, hullboost: 0.8 }],
    properties: { hullMass: 25, baseArmour: 60, baseShieldStrength: 40, name: 'Sidewinder' },
  },
};
...
const MODULES: RawModuleItem[] = [
  ...
  { data: [{ id: '26', grp: 'hr', class: 1, rating: 'E', mass: 2, hullreinforcement: 80, symbol: 'Int_HullReinforcement_Size1_Class1' }] },
  {
    data: [
      {
        id: '1x', grp: 'sg', class: 1, rating: 'A', mass: 1.3, power: 1.68,
        minmass: 13, optmass: 25, maxmass: 63, minmul: 0.7, optmul: 1.2, maxmul: 1.7,
        symbol: 'Int_ShieldGenerator_Size1_Class5',
      },
    ],
  },
  { data: [{ id: '08', grp: 'sb', class: 0, rating: 'E', mass: 0.5, power: 0.2, shieldboost: 0.04, symbol: 'Hpt_ShieldBooster_Size0_Class1' }] },
  {
    data: [
      {
        id: 'w1', grp: 'pl', class: 1, rating: 'F', mount: 'F', mass: 2, power: 0.39,
        damage: 2.05, fireint: 0.5, damagedist: { T: 1 }, symbol: 'Hpt_PulseLaser_Fixed_Small',
      },
    ],
  },
  {
    data: [
      {
        id: 'w2', grp: 'mc', class: 1, rating: 'F', mount: 'F', mass: 2, power: 0.28,
        damage: 4, fireint: 0.5, damagedist: { K: 0.9, T: 0.1 }, symbol: 'Hpt_MultiCannon_Fixed_Small',
      },
    ],
  },
];
...
describe('shields', () => {
  const withGenerator = (extra: FittedModule[] = []): ShipBuild => ({
    ...STOCK,
    modules: [
      ...STOCK.modules.map((m) => (m.group === 'internal' && m.index === 1 ? { ...m, moduleId: '1x' } : m)),
      ...extra,
    ],
  });

  it('MANDATORY: no generator is null, never zero', () => {
    /*
     * A shieldless ship is a deliberate choice — explorers and some miners fly one. Zero would be a
     * claim about a broken build rather than a description of an intentional one.
     */
    expect(computeStats(STOCK, catalogue)?.shields).toBeNull();
  });

  it('applies the generator curve against unladen mass', () => {
    /*
     * The Sidewinder is 38.9 t against a generator whose optimal mass is 25 and maximum 63, so it
     * sits on the upper half of the curve: optmul 1.2 falling towards maxmul 1.7 — no, TOWARDS it,
     * which on this generator means the multiplier rises. 60 base shield × that.
     */
    const stats = computeStats(withGenerator(), catalogue);
    expect(stats?.shields).toBeGreaterThan(0);
  });

  it('MANDATORY: shields curve on UNLADEN mass, not laden', () => {
    /*
     * The generator sizes itself against the hull. A full cargo bay does not weaken the shield, and
     * using laden mass would under-report every trader on the roster.
     */
    const empty = computeStats(withGenerator(), catalogue);

    // Same ship, more cargo. Laden mass changes; the shield must not.
    const laden = computeStats(
      {
        ...withGenerator(),
        modules: withGenerator().modules.map((m) =>
          m.group === 'internal' && m.index === 0 ? { ...m, moduleId: '02' } : m,
        ),
      },
      catalogue,
    );

    expect(laden?.shields).toBe(empty?.shields);
  });

  it('MANDATORY: boosters have diminishing returns', () => {
    /*
     * Six boosters are not six times one. The game applies an exponential falloff, so stacking them
     * is a deliberate trade — and a naive sum would report a shield strength no ship achieves.
     */
    const one = computeStats(withGenerator([slot('hardpoint', 2, '08', 0)]), catalogue)?.shields ?? 0;
    const base = computeStats(withGenerator(), catalogue)?.shields ?? 0;
    const four = computeStats(
      withGenerator([
        slot('hardpoint', 2, '08', 0),
        slot('hardpoint', 3, '08', 0),
        slot('hardpoint', 4, '08', 0),
        slot('hardpoint', 5, '08', 0),
      ]),
      catalogue,
    )?.shields ?? 0;

    const first = one - base;
    const allFour = four - base;

    expect(first).toBeGreaterThan(0);
    // Four boosters add more than one, but less than four times as much.
    expect(allFour).toBeGreaterThan(first);
    expect(allFour).toBeLessThan(first * 4);
  });
});

describe('damage', () => {
  const armed = (...ids: string[]): ShipBuild => ({
    ...STOCK,
    modules: [...STOCK.modules, ...ids.map((id, i) => slot('hardpoint', i, id, 1))],
  });

  it('MANDATORY: an unarmed ship is null, not zero DPS', () => {
    // A trader carrying no guns is a choice, not a gunship that does nothing.
    expect(computeStats(STOCK, catalogue)?.dps).toBeNull();
    expect(computeStats(STOCK, catalogue)?.damageByType).toBeNull();
  });

  it('is damage over the fire interval', () => {
    // 2.05 damage every 0.5s = 4.1 dps.
    expect(computeStats(armed('w1'), catalogue)?.dps).toBeCloseTo(4.1, 2);
  });

  it('adds every weapon', () => {
    // 4.1 + 8.0.
    expect(computeStats(armed('w1', 'w2'), catalogue)?.dps).toBeCloseTo(12.1, 2);
  });

  it('MANDATORY: splits by damage type', () => {
    /*
     * Keeping the split is what lets an answer say "mostly thermal, so it will struggle against a
     * hull-tanked target" instead of quoting one number that hides the entire question.
     */
    const types = computeStats(armed('w1', 'w2'), catalogue)?.damageByType ?? {};

    expect(types['T']).toBeCloseTo(4.1 + 0.8, 2);
    expect(types['K']).toBeCloseTo(7.2, 2);
  });
});
