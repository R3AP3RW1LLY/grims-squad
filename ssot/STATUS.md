...
| D21 | **Licence for a public repository.** `ssot/00-charter/constraints.md` records "our own code — to be chosen by the human", which was fine while the repo was private. It is now public, and a public repo with no `LICENSE` file defaults to **all rights reserved** — readable, but nobody may legally reuse it. That may be exactly what you want. If you would rather permit reuse, MIT or AGPL-3.0 are the sane options, but note the project must stay non-commercial regardless (Frontier/Coriolis data). Separately, the **EDMC plugin must ship from a public repo with readable source** (ADR-014) — I would split it out at P3.8 rather than licence the whole monorepo for it. | P3.8 | 2026-07-26 |
